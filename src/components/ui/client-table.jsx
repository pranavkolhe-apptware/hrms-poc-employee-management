...
//             <TableHead>Managing Executive</TableHead>
...
export default function ClientTable() {
  ...
  // Dummy client data without companyName
  ...
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [deleteClientId, setDeleteClientId] = useState(null);
  // newClient without company field
  ...
  // Filter clients by search text (name or id)
  const filteredClients = clients.filter((client) => {
    return (
      client.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      client.id.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const handleEditSubmit = (e) => {
    ...
    const updatedClient = {
      ...editingClient,
      name: formData.get("name"),
      managingExecutive: formData.get("managingExecutive"),
  ...
  const handleAddSubmit = (e) => {
    ...
    setClients([
      ...clients,
      { id, ...newClient, projects: projectsArr, dateAdded: new Date() },
    ]);
    toast.success("New client added successfully!");
    setIsAddDialogOpen(false);
    setNewClient({
          ...
          <DialogContent>
            ...
            <form onSubmit={handleEditSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="managingExecutive">Managing Executive</Label>
                  <Input
                    id="managingExecutive"
                    name="managingExecutive"
                    defaultValue={editingClient?.managingExecutive}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="workLocation"
                    name="workLocation"
                    defaultValue={editingClient?.workLocation}
                    required
                  />
                </div>
                {/* <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    id="managingExecutive"
                    name="managingExecutive"
                    value={newClient.managingExecutive}
                    onChange={(e) => setNewClient({ ...newClient, managingExecutive: e.target.value })}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="workLocation"
                    name="workLocation"
                    value={newClient.workLocation}
                    onChange={(e) => setNewClient({ ...newClient, workLocation: e.target.value })}
                    required
                  />
                </div>
                {/* <div>
